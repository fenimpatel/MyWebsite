/*
    ...
    Updated: November 13, 2019 at 10:05 AM by Fenim Patel
...
function createMultiplicationTable(startingValueOfMultiplier, endingValueOfMultiplier, startingValueOfMultiplicand, endingValueOfMultiplicand) {
    ...
    for(var i = startingValueOfMultiplicand - 1; i <= endingValueOfMultiplicand; i++) {
        ...
        for(var j = startingValueOfMultiplier - 1; j <= endingValueOfMultiplier; j++) {
            ...
            if(rowOne == true) { // if rowOne is equal to true then execute the statements inside the if condition
                ...
                if(columnOne == true) { // if columnOne is equal to true then execute the statements inside the if condition
                    // element named 'th' is created and assigned to multiplicationTableCell
                    multiplicationTableCell = document.createElement('th');
...
function appendOrReplaceElement(element, parent) {
    ...
    // of element is assigned to previousElement and parent which a member of previousElement has equal value
...
var HandleFormForMultiplicationTable = (function() {
    ...
    var validate = function() { // anonymous function is assigned to a variable validate
        ...
        // and parseInt() which takes arguments starting's member named value and 10 is called so that it parses string and returns integer is
        // greater than parseInt() which takes arguments ending's member named value and 10 is called so that it parses string and returns integer
        // then execute the statements inside the if condition
        if(starting.length !== 0 && ending.length !== 0 && parseInt(starting.value, 10) > parseInt(ending.value, 10)) {
